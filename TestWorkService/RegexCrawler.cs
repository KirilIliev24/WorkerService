    ...
    class RegexCrawler
    {
        ...
        public async Task<List<string>> getAllMetaTags(string website)
        {
            ...
            try
            {
                ...
                //MatchCollection colection = await Task.Run(() => Regex.Matches(websiteString, "^<meta +property=\"(.*)\" +content=\"([^\"]+)\" *\\/> $"));
                //MatchCollection colection = await Task.Run(() => Regex.Matches(websiteString, "/<meta content=\"([^\"]+)\" >/ "));

                for (int i = 0; i < colection.Count; i++)
                {
                    string metaTag = colection[i].Groups[1].Value;
                    metaTag = Regex.Replace(metaTag, "<!*[^<>]*>", " ").Trim();
                    metaTag = Regex.Replace(metaTag, "(<.*?>\\s*)+", " ").Trim();
                    metaTag = Regex.Replace(metaTag, "\\s+", " ");
                    //linkUrl = Regex.Replace("r\"[^a - zA - Z] + \"", "", linkUrl);
